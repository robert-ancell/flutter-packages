...
class LinuxGeneratorAdapter implements GeneratorAdapter {
  ...
  void generate(
      StringSink sink, PigeonOptions options, Root root, FileType fileType) {
    final LinuxOptions linuxOptions = options.linuxOptions ?? const LinuxOptions();
  ...
  IOSink? shouldGenerate(PigeonOptions options, FileType fileType) {
    if (fileType == FileType.source) {
      return _openSink(options.linuxSourceOut, basePath: options.basePath ?? '');
    } else {
      return _openSink(options.linuxHeaderOut, basePath: options.basePath ?? '');
    }
  }
