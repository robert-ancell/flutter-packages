...
class LinuxHeaderGenerator extends StructuredGenerator<LinuxOptions> {
      ...
      _writeAccessBlock(indent, () {
        ...
        for (final NamedType field in orderedFields) {
          ...
          final HostDatatype baseDatatype = getFieldHostDatatype(
              field, root.classes, root.enums, _baseLinuxTypeForBuiltinDartType);
  ...
  void writeFlutterApi(
    ...
    const List<String> generatedMessages = <String>[
      ' Generated class from Pigeon that represents Flutter messages that can be called from Linux.'
    ];
  ...
  void _writeClassConstructor(Root root, Indent indent, Class klass,
    ...
    final List<String> paramStrings = params.map((NamedType param) {
      final HostDatatype hostDatatype = getFieldHostDatatype(
          param, root.classes, root.enums, _baseLinuxTypeForBuiltinDartType);
  ...
  void _writeCodec(
      LinuxOptions generatorOptions, Root root, Indent indent, Api api) {
    assert(getCodecClasses(api, root).isNotEmpty);
    final String codeSerializerName = _getCodecSerializerName(api);
    indent
        .write('class $codeSerializerName');
    indent.addScoped('{', '};', () {
      _writeAccessBlock(indent, () {
...
/// Writes Linux source (.cpp) file to sink.
class LinuxSourceGenerator extends StructuredGenerator<LinuxOptions> {
  ...
  void writeDataClass(
    ...
    // Getters and setters.
    for (final NamedType field in orderedFields) {
      _writeLinuxSourceClassField(generatorOptions, root, indent, klass, field);
    }
    ...
    _writeFunctionDefinition(indent, 'ToEncodableList',
      ...
      for (final NamedType field in getFieldsInSerializationOrder(klass)) {
        final HostDatatype hostDatatype = getFieldHostDatatype(field,
            root.classes, root.enums, _shortBaseLinuxTypeForBuiltinDartType);
  ...
  void writeFlutterApi(
    ...
    indent.writeln(
        '$_commentPrefix Generated class from Pigeon that represents Flutter messages that can be called from Linux.');
    ...
    for (final Method func in api.methods) {
      ...
      final Iterable<_HostNamedType> hostParameters =
        ...
        final HostDatatype hostType = getFieldHostDatatype(
            arg, root.classes, root.enums, _shortBaseLinuxTypeForBuiltinDartType);
  ...
  void writeHostApi(
    ...
    _writeFunctionDefinition(indent, 'get_codec',
        scope: api.name,
        returnType: 'FlStandardMessageCodec*', body: () {
      indent.writeln(
          'return fl_standard_message_codec_new();');
    });
    indent.writeln(
        '$_commentPrefix Sets up an instance of `${api.name}` to handle messages through the `binary_messenger`.');
            ...
            indent.addScoped('{', '});', () {
              indent.writeScoped('try {', '}', () {
                ...
                if (method.arguments.isNotEmpty) {
                  ...
                  enumerate(method.arguments, (int index, NamedType arg) {
                    final HostDatatype hostType = getHostDatatype(
                        ...
                        root.enums,
                        (TypeDeclaration x) =>
                            _shortBaseLinuxTypeForBuiltinDartType(x));
                ...
                final HostDatatype returnType = getHostDatatype(
                    ...
                    root.classes,
                    root.enums,
                    _shortBaseLinuxTypeForBuiltinDartType);
                final String returnTypeName = _hostApiReturnType(returnType);
                if (method.isAsynchronous) {
                  methodArgument.add(
...
\terror.details()
  ...
  void _writeLinuxSourceClassField(LinuxOptions generatorOptions, Root root,
      Indent indent, Class klass, NamedType field) {
    final HostDatatype hostDatatype = getFieldHostDatatype(
        field, root.classes, root.enums, _shortBaseLinuxTypeForBuiltinDartType);
    ...
    if (hostDatatype.isNullable) {
      // Write the non-nullable variant; see _writeLinuxHeaderDataClass.
  ...
  String _wrapResponse(Indent indent, Root root, TypeDeclaration returnType,
    ...
    if (returnType.isVoid) {
      ...
      final HostDatatype hostType = getHostDatatype(returnType, root.classes,
          root.enums, _shortBaseLinuxTypeForBuiltinDartType);
...
${prefix}reply(EncodableValue(std::move(wrapped)));''';
  ...
  /// A wrapper for [_baseLinuxTypeForBuiltinDartType] that generated Flutter
  /// types without the namespace, since the implementation file uses `using`
  /// directives.
  String? _shortBaseLinuxTypeForBuiltinDartType(TypeDeclaration type) {
    return _baseLinuxTypeForBuiltinDartType(type, includeFlutterNamespace: false);
  }
}
...
/// should generally be passed by value rather than pointer/reference) in Linux.
bool _isPodType(HostDatatype type) {
  return !_isReferenceType(type.datatype);
}

String? _baseLinuxTypeForBuiltinDartType(
  TypeDeclaration type, {
  bool includeFlutterNamespace = true,
}) {
  final String flutterNamespace = includeFlutterNamespace ? 'flutter::' : '';
  final Map<String, String> linuxTypeForDartTypeMap = <String, String>{
    'void': 'void',
    'bool': 'bool',
    'int': 'int64_t',
    'String': 'std::string',
  ...
  if (linuxTypeForDartTypeMap.containsKey(type.baseName)) {
    return linuxTypeForDartTypeMap[type.baseName];
  } else {
    return null;
  }
}

/// Returns the Linux type to use in a value context (variable declaration,
/// pass-by-value, etc.) for the given Linux base type.
...
/// Returns the Linux type to use in an argument context without ownership
...
/// Returns the Linux type to use for arguments to a host API. This is slightly
...
/// Returns the Linux type to use for arguments to a Flutter API.
...
/// Returns the Linux type to use for the return of a getter for a field of type
/// [type].
...
/// Returns the Linux type to use for the return of a host API method returning
/// [type].
...
/// Returns the Linux type to use for the paramer to the asynchronous "return"
...
void _writeAccessBlock(
    Indent indent, void Function() body) {
  indent.addScoped('', '', body);
}

/// Validates an AST to make sure the Linux generator supports everything.
List<Error> validateLinux(LinuxOptions options, Root root) {
    ...
    for (final Method method in api.methods) {
      for (final NamedType arg in method.arguments) {
        if (isEnum(root, arg.type)) {
          ...
          result.add(Error(
              message:
                  "Nullable enum types aren't supported in Linux arguments in method:${api.name}.${method.name} argument:(${arg.type.baseName} ${arg.name})."));
        }
      }
    }
  }
